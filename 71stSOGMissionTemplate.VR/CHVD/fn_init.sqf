...
[] spawn {
	...
	//player addAction ["<t color='#ffc600'>View Distance Settings</t>", CHVD_fnc_openDialog, [], -99, false, true];
	//player addEventHandler ["Respawn", {player addAction ["<t color='#ffc600'>View Distance Settings</t>", CHVD_fnc_openDialog, [], -99, false, true]}];
	
	if (local ace_player || hasInterface) then {

		_action = ["settings","View Distance Settings","",{call CHVD_fnc_openDialog;},{[player] call ace_common_fnc_canInteract}] call ace_interact_menu_fnc_createAction;
